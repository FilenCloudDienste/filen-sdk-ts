...
export class User {
	...
	public async changePassword({ currentPassword, newPassword }: { currentPassword: string; newPassword: string }): Promise<void> {
		...
		const newMasterKeys: string[] = [...this.sdkConfig.masterKeys!, derivedNew.derivedMasterKeys]
	 ...
	 */
	public async events({ timestamp, filter }: { timestamp?: number; filter?: "all" }): Promise<UserEvent[]> {
